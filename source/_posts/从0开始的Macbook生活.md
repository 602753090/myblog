...
- [x] **homebrew** - 一个类似于包管理器的软件，可以用来在终端直接安装软件。好处是只需要在终端输入`brew upgrade`就可以一键更新所有用honebrew安装的软件，安装软件时可以用`brew search <软件名>`先搜索有没有这个软件，有的话使用`brew install <软件名>`安装。
...
- [x] **ParallelDesktop** - 虚拟机软件，可以用来运行Windows系统。
- [ ] **CrossOver** - 用来直接在Mac上运行Windows软件。
